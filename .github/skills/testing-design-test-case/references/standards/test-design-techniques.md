...
description: "Guides selection and application of test-design techniques such as equivalence partitioning, boundary analysis, decision tables, state transitions, use-case testing, pairwise testing, and error guessing. Use when deriving risk-based scenarios from requirements. Do not use for test execution or test-case review."
---
...
Read the requirement, acceptance criteria, input constraints, business rules, workflow states, and integration boundaries before selecting a technique. Ask when the behavior or scope is ambiguous. Assume no technique is mandatory unless the requirement contains a matching risk or rule, and record any coverage gap as an assumption or open question.

## Workflow

1. **Classify the behavior.** Input: requirement behavior and risk context. Action: identify inputs, limits, categories, rule combinations, state changes, journeys, and known defect risks. Output: technique-selection factors.
2. **Select techniques.** Input: technique-selection factors. Action: choose equivalence partitioning, boundary value analysis, decision tables, state transition, use-case, pairwise, or error-guessing techniques as appropriate. Output: a non-duplicated technique plan.
3. **Derive scenarios.** Input: the technique plan and requirement constraints. Action: generate representative valid, invalid, boundary, combination, transition, and error scenarios. Output: traceable candidate test cases.
4. **Check coverage.** Input: candidate cases. Action: verify that each relevant rule and risk is covered and that similar cases are consolidated. Output: an approved scenario set or visible gaps.

## Output Contract

The technique selection must be explainable from the requirement. Generated scenarios must identify the primary technique, cover relevant partitions or boundaries, remain atomic and traceable, and avoid unnecessary overlap.
...
Use [`test-design-approach.md`](test-design-approach.md) for scenario planning and [`testing-standard.md`](testing-standard.md) for shared traceability, quality, security, accessibility, and non-functional policy.
